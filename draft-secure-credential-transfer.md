...
# 3. Credential transfer workflows {#workflows}
...
## 3. 1. Stateless workflow {#statless-workflow}
...
*Sample flow of stateless process:*
...
## 3.2. Stateful workflow {#stateful-workflow}
...
# 5. HTTP Headers: X-Correlation-ID {#http-headers}
...
# 6. HTTP access methods {#api-endpoints}

## 6.1. CreateMailbox{#create-mailbox}
### 6.1.1. endpoint: POST  /{version}/mailbox
...
### 6.1.4. Request body - complex structure, including:
...
- notificationToken (Object, Optional) - optional notification token used to notify an appropriate remote device that the mailbox data has been updated. Data structure includes the following:
	...
	"ApplePushToken example” {"name":"com.apple.apns","tokenData":"APNS1234...QW"}
...
## 6.2. UpdateMailbox {#update-mailbox}
### 6.2.1. endpoint: PUT  /{version}/mailbox/{mailboxIdentifier}
...
## 6.3. DeleteMailbox {#delete-mailbox}
### 6.3.1. endpoint: DELETE /{version}/mailbox/{mailboxIdentifier}
...
## 6.4. ReadDisplayInformationFromMailbox {#read-display-information}

### 6.4.1. endpoint: GET /{version}/mailbox/{mailboxIdentifier}
...
## 6.5. ReadSecureContentFromMailbox {#read-secure-content}
### 6.5.1. endpoint: POST /{version}/mailbox/{mailboxIdentifier}
An application running on a remote device can invoke this API on Relay Server to to retrieve secure data content from a mailbox (encrypted data specific to a Provisioning Information Provider).
...
# 7. Encryption format {#encryption-format}
...
Currently proposed formats: 

- "AES-128/GCM/NoPadding" - AES symmetric encryption algorithm with key length 128 bit, in GCM mode with no padding.
- "AES-256/GCM/NoPadding" - AES symmetric encryption algorithm with key length 256 bit, in GCM mode with no padding.
